...
FPC_BEGIN
	...
	if (dw == INVALID_HANDLE_VALUE) {
		...
		FPC_DLLFUNC(LazExitProcess,rtl)(1);
	}
...
// ----------------------------------------------------------
uint32_t FPC_DLLFUNC(LengthString,rtl)(LazSTRING s)
FPC_BEGIN
    uint32_t len = 0;
	char *buffer = new char[128];
    for (len; s[len] != '\0'; ++len);
	itoa(len,buffer,10);
    MessageBoxA(0,buffer,"info len 11110000",MB_OK);
	return len;
FPC_END
...
// ----------------------------------------------------------
void FPC_DLLFUNC(LowerCase,rtl)(LazSTRING *ret, LazSTRING s)
FPC_BEGIN
    for (int i = 0; s[i] != '\0'; i++) {
        if (s[i] >= 'A' && s[i] <= 'Z') {
            s[i] = s[i] + 32;
...
// ----------------------------------------------------------
void FPC_DLLFUNC(UpperCase,rtl)(LazSTRING *ret, LazSTRING s)
FPC_BEGIN
    for (int i = 0; s[i] != '\0'; i++) {
        if ((s[i] >= 'a') && (s[i] <= 'z')) {
            s[i] = s[i] - 32;
